...
void Seesaw::setup() {
  ...
  if (this->readbuf(SEESAW_STATUS, SEESAW_STATUS_OPTIONS, buf, 4) == i2c::ERROR_OK)
    this->options_ = (uint32_t(buf[0]) << 24) | (uint32_t(buf[1]) << 16) | (uint32_t(buf[2]) << 8) | buf[3];
}
...
void Seesaw::dump_config() {
  ...
  ESP_LOGCONFIG(TAG, "  Version: 20%02u-%02u-%02u %u", static_cast<unsigned int>(v & 0x3f),
                static_cast<unsigned int>((v >> 7) & 0xf), static_cast<unsigned int>((v >> 11) & 0x1f),
                static_cast<unsigned int>(v >> 16));
  ESP_LOGCONFIG(TAG, "  Options: %08x", static_cast<unsigned int>(this->options_));
}
...
bool Seesaw::digital_read(uint8_t pin) {
  ...
  uint32_t ret = (uint32_t(buf[0]) << 24) | (uint32_t(buf[1]) << 16) | (uint32_t(buf[2]) << 8) | buf[3];
  return ret & pins;
}
...
void Seesaw::setup_neopixel(uint8_t pin, uint16_t num_leds) {
